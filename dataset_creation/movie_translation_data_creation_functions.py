...
def extract_video_clips(language, actor, metadata, youtube_videos_dir=config.YOUTUBE_VIDEOS_DIR, verbose=False):
...
def extract_face_frames_from_video(video_file, detector, predictor,
                                   save_with_blackened_mouths_and_polygons=True, save_gif=True,
    ...
    [optional] Save landmarks in MOVIE_TRANSLATION_DATASET_DIR/landmarks/language/actor/video_file
    '''
    ...
    for frame_number, frame in tqdm.tqdm(enumerate(video_frames), total=len(video_frames)):
        ...
        if landmarks:
            ...
            if save_with_blackened_mouths_and_polygons:
                ...
                # Init face_with_blackened_mouth_and_mouth_polygon
